...
class Bias_Node(Module):

    def __init__(self, in_features):
        super(CWNlinear, self).__init__()
        self.in_features = in_features
        self.out_features = in_features # we are only shifting variables
        self.bias = Parameter(torch.Tensor(out_features))
...
class Dense_Epsilon(Module):
    def __init__(self, dim=6, batch_size=16):
        super(Dense_Epsilon, self).__init__()
        # define epsilon NN (start with 3 layer dense network)
        self.batch_size = batch_size
        dim_hid = 2 * dim
        self.eps_dense = nn.Sequential(
                nn.Sigmoid(),
                nn.PReLU(),
